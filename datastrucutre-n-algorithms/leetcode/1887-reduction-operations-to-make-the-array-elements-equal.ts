...
function reductionOperations(nums: number[]): number {
  ...
  let sortedMap: Map<number, number> = new Map();
  sortedNums.forEach((num) => {
    sortedMap.set(num, (sortedMap.get(num) || 0) + 1);
  });
  if (sortedMap.size === 1) {
    return 0;
  }
  let result = 0;
  const uniqueNums = Array.from(sortedMap.keys());
  for (let i = 1; i < uniqueNums.length; i++) {
    result = result + (sortedMap.get(uniqueNums[i]) || 0) * i;
  }
  return result;
};
